...
class Node:
    ...
    def update_longest_chain(self, new_block_tree_node):
        if (new_block_tree_node.prevBlock != self.current_max_height_tree_node):
            print("WHAT??? Its returning txs to the global tx pool???")
    ...
    def mining(self, i):
        while(True): #global variable
            #sleep(random)
            print("q len is: ", self.q.qsize())
            if (not self.q.empty()):
                new_block = self.q.get()
                self.receive_block(new_block)
                print("this node is mining: ", i)
                print("q len is: ", self.q.qsize())
